...
trait ValidatesForm
{
    ...
    protected function scaffoldRules()
    {
        ...
        foreach ($schema->columns($table) as $column) {
            $name = $column->getName();
    ...
    {
        foreach ($foreign = app('scaffold.schema')->foreignKeys($eloquent->getTable()) as $foreign) {
    ...
    {
        foreach (app('scaffold.schema')->indexes($eloquent->getTable()) as $indexName => $index) {
